...
前面九节基本上覆盖了Shell编程最基本的内容，这一节我们来动手写两个脚本，一方面是把我们之前学到的东西复习一下，串联起来。另一方面，是将之前没有覆盖到的几个常用命令介绍一下。

我们的第一个任务是在当前目录下，新建一个脚本`final-script.sh`。然后在脚本里新建一个文件夹`final`，在新文件夹里批量新建十个`.txt`文件，命名规则为`final-test-n.txt`。每个文件的文档内容为当前的日期以及附加的一行字`"This is file n."`。
...
<li><a href="http://www.uccs.edu/~ahitchco/grep/">Drew's Grep Tutorial</a></li>
</ul>
